...
def diamond(cap=1, middle=1):
    """Raute S -> A, B -> T mit Querkante A -> B. Kanten in der Reihenfolge S->A, S->B, A->B, A->T, B->T: eine Tiefensuche geht
    zuerst S-A-B-T (Engpass = Querkante) und findet den Rest nur über die Rückkante B -> A."""
    names = ["S", "T", "A", "B"]
    pos = [(50, 90), (50, 10), (20, 50), (80, 50)]
    arcs = [(0, 2, cap, 1), (0, 3, cap, 2), (2, 3, middle, 1), (2, 1, cap, 2), (3, 1, cap, 1)]
    return _teaching(names, pos, arcs)


def trap(m=1000):
    """Dieselbe Raute mit großen Kapazitäten m: die Querkante trägt nur 1 - die Falle des Ford-Fulkerson-Verfahrens."""
    return diamond(cap=m, middle=1)
...
NETS = {
    "random": lambda: None,
    "detour": lambda: diamond(),
    "trap": lambda: trap(1000),
